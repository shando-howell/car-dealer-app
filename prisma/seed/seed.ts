...
async function seedTaxonomy(prisma: PrismaClient) {
        ...
        fs.createReadStream("taxonomy.csv")
            ...
            .on("data", async (row: {[index: string]: string }) => {
                eachRow.push({
                    ...
                    yearEnd: row.YearEnd ? Number(row.Year_End) : new Date().getFullYear(),
                });
            }).on("error", (error) => {
                console.log({ error });
                reject(error);
    ...
    const makePromises = Object.entries(result).map(([name]) => {
        return prisma.make.upsert({
            ...
            update: {
                name,
                image: `https://vl.imgix.net/img/${name.replace(/\$+/g, '-').toLowerCase()}-logo.png?auto=format.compress`
            },
            create: {
                name,
                image: `https://vl.imgix.net/img/${name.replace(/\$+/g, '-').toLowerCase()}-logo.png?auto=format.compress`
    ...
    console.log(`Seeded db with ${makes.length} models`);
    ...
    async function insertInBatches<TUpsertArgs>(
        ...
        for(let i = 0; i < items.length; i += batchSize) {
            const batch = items.slice(1, i + batchSize);
